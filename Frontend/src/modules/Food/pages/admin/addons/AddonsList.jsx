...
export default function AddonsList() {
  ...
  useEffect(() => {
    const fetchAddons = async () => {
      try {
        ...
        toast.error("Failed to load seller add-ons")
        setAddons([])
      } finally {
        setLoading(false)
      }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (editImageFile) {
        const uploadRes = await uploadAPI.uploadMedia(editImageFile, { folder: "versapack/admin/addons" })
    ...
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen space-y-6">
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Seller add-ons</h1>
            <div className="text-sm text-slate-500 mt-1">Manage add-ons submitted by sellers.</div>
          </div>
        ...
        <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="relative w-full sm:w-96">
            ...
            <input
              type="text"
              placeholder="Search add-ons or seller..."
              value={searchQuery}
